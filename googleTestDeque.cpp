...
const int SIZE_TEST_FLAG1 = 10000;
const int SIZE_TEST_FLAG2 = 100000;
const int SIZE_TEST_FLAG3 = 300000;
...
TEST(DequeTest, testSpeed1) {
    Deque<int> d;
    time_t start = clock();
    for (size_t i = 0; i < SIZE_TEST_FLAG1; ++i) {
        if (rand() % (10) < 10 / 2) {
            d.push_front(i);
        }
        else {
    ...
    time_t finish = clock();
    size_t timeFlag1 = (finish - start) / CLOCKS_PER_SEC;
    start = clock();
    for (size_t i = 0; i < SIZE_TEST_FLAG2; ++i) {
        if (rand() % (10) < 10 / 2) {
            d.push_front(i);
        }
        else {
    ...
    finish = clock();
    size_t timeFlag2 = (finish - start) / CLOCKS_PER_SEC;
    ASSERT_TRUE(timeFlag1 * 10 >= timeFlag2);
}

TEST(DequeTest, testSpeed2) {
    Deque<int> d;
    time_t start = clock();
    for (size_t i = 0; i < SIZE_TEST_FLAG2; ++i) {
        if (rand() % (10) < 10 / 2) {
    ...
    size_t timeFlag1 = (finish - start) / CLOCKS_PER_SEC;
    start = clock();
    for (size_t i = 0; i < SIZE_TEST_FLAG3; ++i) {
        if (rand() % (10) < 10 / 2) {
            d.push_front(i);
        }
        else {
    ...
    finish = clock();
    size_t timeFlag2 = (finish - start) / CLOCKS_PER_SEC;
    ASSERT_TRUE(timeFlag1 * 2 >= timeFlag2);
}
